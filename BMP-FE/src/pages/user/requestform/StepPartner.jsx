...
const VEHICLE_OPTIONS = [
  ...
  { type: "SUV",        icon: <Car size={16} /> },
  { type: "Van",        icon: <Truck size={16} /> },
  { type: "Mini Truck", icon: <Truck size={16} /> },
  { type: "Truck",      icon: <Truck size={16} /> },
  { type: "Tempo",      icon: <Truck size={16} /> },
...
const StepPartner = ({ data, updateFields, onNext, onBack, parcelId }) => {
  ...
  const filteredTravelers = selectedVehicle
    ? travellers.filter((t) => t.vehicleType?.toLowerCase() === selectedVehicle.toLowerCase())
    : travellers;
              ...
              {showVehicleMenu && (
                <div className="absolute right-0 z-30 mt-2 bg-white border shadow-lg w-44 rounded-xl">
                  {VEHICLE_OPTIONS.map((v) => {
                    const isActive = selectedVehicle?.toLowerCase() === v.type.toLowerCase();
                    return (
                      ...
                      >
                        <span className={isActive ? "text-blue-600" : "text-primary"}>{v.icon}</span>
                        <span>{v.type}</span>
                  ...
                  <button onClick={() => { setSelectedVehicle(null); setShowVehicleMenu(false); }}
                    className="w-full px-4 py-2 text-xs text-gray-500 hover:bg-gray-50 border-t"
                  >
                    Clear filter
                  </button>
                </div>
