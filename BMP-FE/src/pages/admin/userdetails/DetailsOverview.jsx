...
const DetailsOverview = ({ user = {} }) => {
  ...
  const VerifyCard = ({ label, verified }) => (
    <div className={`border p-4 rounded-lg flex items-center justify-between ${
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">{verified ? "Verified" : "Not Verified"}</p>
      </div>
      {verified
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        <h3 className="font-semibold mb-4 text-lg">Verification Status</h3>
      ...
      <div className="mt-6 bg-purple-50 border border-purple-200 rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div><p className="text-lg font-semibold">{Number(user.total_bookings) || 0}</p><p className="text-sm text-gray-500">Total Bookings</p></div>
          <div><p className="text-lg font-semibold">₹{Number(user.total_spent || 0).toLocaleString()}</p><p className="text-sm text-gray-500">Amount Spent</p></div>
          <div><p className="text-lg font-semibold">{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : "—"}</p><p className="text-sm text-gray-500">Joined Date</p></div>
        </div>
      </div>
