...
export default function TravelRoute2() {
  ...
  const validate = () => {
    ...
    if (formData.transportMode === "train") {
      ...
      else if (!/^\d{4,5}$/.test(formData.trainNumber)) e.trainNumber = "Train number must be 4 or 5 digits";
  ...
  const handleNext = () => {
    ...
    StorageService.setData(APPLICATION_CONSTANTS.STORAGE.ROUTE_STEP_2, formData);
