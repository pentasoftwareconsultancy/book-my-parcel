...
const FIELDS = [
  ...
  { name: "pincode", label: "Pincode *", placeholder: "e.g. 400001", maxLength: 6, inputMode: "numeric" },
  ...
  return (
    <div className="bg-white rounded-2xl p-6 mb-4 border border-gray-100 shadow-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {FIELDS.map(({ name, label: fLabel, placeholder, maxLength, inputMode }) => (
          <Controller key={name} name={`${fieldName}.${name}`} control={control} render={({ field }) => (
            <InputField label={fLabel} error={!!fieldErrors[name]} helperText={fieldErrors[name]?.message}>
              <TextInput
                ...
                inputMode={inputMode}
                onChange={(e) => {
                  // Enforce maxLength for pincode (type=text ignores maxLength on some browsers)
