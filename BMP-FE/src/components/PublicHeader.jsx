...
const Navbar = () => {
  ...
  // Only fetch notifications when authenticated — avoids 401 triggering logout
  ...
  // Show text+icons when on public homepage (regardless of auth state)
  const isOnHomepage = location.pathname === "/" || location.pathname === RoutePath.PUBLIC_HOME;
  const showTextButtons = isOnHomepage && !isMobile; // always text on homepage, icons only on dashboard/profile
  ...
  useEffect(() => {
    ...
    else if (activeRole === "TRAVELLER") navigate(RoutePath.TRAVELLER_NOTIFICATIONS); // add if needed
...
const handleProfileRedirect = () => {
  ...
  return (
    <>
      {/* ══════════════════════════════════════════
          ALL sizing via <style> + media queries.
          This is 100% reliable — no Tailwind, no JS.
         ══════════════════════════════════════════ */}
      <style>{`
        ...
        /* ── Hide/show helpers (replaces sm:hidden / sm:flex) ── */
      ...
      <header style={{
        ...
        <div style={{ width: "100%", padding: isAuthenticated ? "0 16px 0 25px" : "0 12px", boxSizing: "border-box", maxWidth: isAuthenticated ? "none" : 1280, margin: isAuthenticated ? "0" : "0 auto" }}>
          <div className={isAuthenticated ? "bmp-bar" : "bmp-bar bmp-bar-pub"}>
            ...
            {/* ── LEFT: Location (authenticated) or Logo + Location (public) ── */}
            <div style={{ display: "flex", alignItems: "center", gap: 20, minWidth: 0 }}>
              ...
              {!isAuthenticated && (
                <>
                  {/* Mobile: plain icon with tooltip, no border */}
                  {isMobile ? (
                    ...
                    /* Desktop: bordered button with text */
                    <button
                      onClick={() => setShowLocationModal(true)}
                      style={{
            ...
            <div className="bmp-group">
              ...
              {isAuthenticated && (
                <>
                  {/* On homepage → text + icon buttons, on dashboard → icon only */}
                  {showTextButtons ? (
                    // ── Homepage: full text buttons ──
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      {user?.activeRole === "INDIVIDUAL" && (                         
                        <button onClick={() => navigate(RoutePath.USER_REQUEST_FORM)} 
                        style={{
                      ...
                      <div style={{ position: "relative" }}>
                        <button
                          className="bmp-btn bmp-btn-bell"
                          onClick={handleNotificationRedirect}
                          onMouseEnter={() => setTooltip("notif")}
                          onMouseLeave={() => setTooltip(null)}
                          aria-label="Notifications"
                        >
                          <Bell />
                          {unreadCount > 0 && (
                            <span className="bmp-badge">{unreadCount > 99 ? "99+" : unreadCount}</span>
                          )}
                        </button>
                      ...
                      <div ref={dropdownRef} style={{ position: "relative" }}>
                        ...
                        {tooltip === "user" && !open && <div className="bmp-tip">Account</div>}
                        {open && (
                          <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 192, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, boxShadow: "0 10px 25px rgba(0,0,0,.10)", overflow: "hidden", zIndex: 50 }}>
                            ...
                            <button onClick={handleLogout} style={{ display: "block", width: "100%", padding: "10px 16px", fontSize: 14, fontWeight: 600, textAlign: "left", color: "#dc2626", background: "transparent", border: "none", cursor: "pointer" }}
                    ...
                    // ── Dashboard/Profile: icon only ──
                    <>
                      {user?.activeRole === "INDIVIDUAL" && (                        
                        <div style={{ position: "relative" }}>
                          <button className="bmp-btn" 
                            onClick={() => navigate(RoutePath.USER_REQUEST_FORM)}
                            onMouseEnter={() => setTooltip("send")} 
                            onMouseLeave={() => setTooltip(null)} aria-label="Send Parcel">
                            <Send />
                          </button>
                      ...
                      <div style={{ position: "relative" }}>
                        <button className="bmp-btn bmp-btn-bell" 
                          onClick={handleNotificationRedirect}
                          onMouseEnter={() => setTooltip("notif")} 
                          onMouseLeave={() => setTooltip(null)} 
                          aria-label="Notifications">
                      ...
                      <div ref={dropdownRef} style={{ position: "relative" }}>
                        <button className="bmp-btn" 
                          onClick={() => setOpen(!open)}
                          onMouseEnter={() => setTooltip("user")} 
                          onMouseLeave={() => setTooltip(null)} aria-label="User menu">
                          <User />
                        </button>
                        {tooltip === "user" && !open && <div className="bmp-tip">Account</div>}
                        {open && (
                          <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 192, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, boxShadow: "0 10px 25px rgba(0,0,0,.10)", overflow: "hidden", zIndex: 50 }}>
                            ...
                            {[
                              ...
                              <button key={label} onClick={action} style={{ display: "block", width: "100%", padding: "10px 16px", fontSize: 14, fontWeight: 600, textAlign: "left", color: "#374151", background: "transparent", border: "none", cursor: "pointer" }}
                                onMouseEnter={e => e.currentTarget.style.background = "#f9fafb"}
                                onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                                {label}
                              </button>
