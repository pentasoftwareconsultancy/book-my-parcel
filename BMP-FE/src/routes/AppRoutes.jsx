...
// ─── Layouts (lazy loaded — each layout pulls in sidebar/header/footer) ──────
...
const BookingCancle = lazy(() => import("../pages/user/BookingCancle"));
...
const OrderSummary = lazy(() => import("../pages/user/requestform/OrderSummary"));
...
const MyRouts = lazy(() => import("../pages/traveler/MyRouts"))
...
// ─── Loading Fallback ─────────────────────────────────────────────────────────
// Full-screen spinner — used only for the initial app shell load
const LoadingFallback = () => (
  <div className="flex items-center justify-center h-screen bg-gray-50">
    <div className="text-center">
      <div className="inline-block w-12 h-12 mb-4 border-t-2 border-b-2 border-blue-500 rounded-full animate-spin"></div>
      <p className="font-medium text-gray-600">Loading...</p>
    </div>
  </div>
);

// Inline spinner — used for individual route transitions so a single failing
// chunk doesn't block the entire app with a full-screen spinner
const PageFallback = () => (
...
// Wrap a route element in its own Suspense boundary so lazy-load failures
// are isolated to that page rather than crashing the whole route tree
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute allowedRoles={[USER_ROLES.INDIVIDUAL]} />}>
          <Route path={RoutePath.USER_BASE} element={<UserLayout />}>
            ...
            <Route path={RoutePath.USER_BOOKING_CANCLE}     element={<S><BookingCancle /></S>} />
        ...
        <Route element={<ProtectedRoute allowedRoles={[USER_ROLES.TRAVELLER]} />}>
          ...
          <Route path={RoutePath.TRAVELER_BASE} element={<TravelerLayout />}>
            ...
            <Route path={RoutePath.TRAVELER_MYROUTES}           element={<S><MyRouts /></S>} />
