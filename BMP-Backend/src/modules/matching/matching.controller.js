...
export async function getAcceptances(req, res) {
  try {
    ...
    // Deduplicate: keep only the best route per traveller (lowest detour_km)
    ...
    for (const acc of formattedAcceptances) {
      ...
      if (!existing || (acc.detour_km ?? Infinity) < (existing.detour_km ?? Infinity)) {
    ...
    if (includePending) {
      ...
      for (const req of pendingRequests) {
        ...
        if (!existing || (req.detour_km ?? Infinity) < (existing.detour_km ?? Infinity)) {
