...
async function findCandidateTravellers(parcelData) {
  try {
    ...
    // never read — the read is now done at the top of the function.
...
/**
 * Convert a time string (HH:MM or HH:MM:SS from PostgreSQL) to minutes since
 * midnight. Using integer comparison is safe across midnight boundaries and
 * immune to the lexicographic ordering bug where "09:30:00" > "09:30".
 */
...
function applyTemporalFilters(routes, parcelData) {
  ...
  // Current time as minutes since midnight — avoids string comparison bugs
  ...
  return routes.filter((route) => {
    if (!route.is_recurring) {
      ...
      if (departureDate.getTime() === today.getTime() && route.departure_time) {
        // FIX: Compare as integers (minutes since midnight), not strings.
        // String comparison breaks when PostgreSQL returns "HH:MM:SS" format.
        const departureMinutes = timeToMinutes(route.departure_time);
        if (departureMinutes !== null && departureMinutes <= currentMinutes) {
          return false;
        }
      }
      ...
      const dayMatches  = Array.isArray(route.recurring_days) && route.recurring_days.includes(dayOfWeek);

      // For recurring routes, even if today's departure has passed, next
      // week's occurrence is still valid — so we always return true here
      // as long as the date range and day match.
